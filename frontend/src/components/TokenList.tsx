...
export default function TokenList({ tokens, onSelect, disabled }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return tokens.filter(
      (t) => t.text.toLowerCase().includes(q) || String(t.token_id).includes(q),
    );
  }, [tokens, query]);
    ...
    <div className={`h-full flex flex-col ${disabled ? "opacity-60 pointer-events-none" : ""}`}>
      ...
      <div className="shrink-0 px-4 py-3 border-b border-zinc-800/40">
        <input
          ...
          className="w-full bg-zinc-900/60 border border-zinc-800 rounded-md px-3 py-1.5 text-xs
        ...
        {filtered.map((t) => {
          ...
          return (
            <button
              key={t.token_id}
              onClick={() => onSelect(t)}
              className="w-full grid grid-cols-[3rem_minmax(5rem,1fr)_1fr_4.5rem_4rem] gap-2 items-center
                px-4 py-1.5 text-left text-xs hover:bg-zinc-800/50 transition-colors group"
            >
