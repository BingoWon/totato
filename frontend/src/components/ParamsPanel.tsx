    ...
    <div className="p-5 space-y-6 text-sm">
      ...
      <Section title="Parameters">
        <div className="space-y-4">
          ...
          <div>
            <label className="text-zinc-400 block mb-2">Top-K</label>
            <input
              type="number"
              min={1}
              max={2000}
              value={topK}
              onChange={(e) => onTopKChange(Math.max(1, parseInt(e.target.value) || 200))}
              className="w-full bg-zinc-900/70 border border-zinc-800 rounded-md px-3 py-1.5 text-xs font-mono
